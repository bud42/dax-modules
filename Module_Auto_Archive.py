...
class Module_Auto_Archive(SessionModule):
    ...
    def archive_prearchive_session(self, src, proj, subj, exp, reset_first=False):
        ...
        post_body = """src={src}&project={proj}&subject={subj}&session={sess}""".format(
            src=src,
            proj=proj,
            subj=subj,
            sess=exp
    ...
    def load_redcap(self):
        ...
        # Load sess events
        self.rc_list = rc.export_records(
        ...
        if self.rc_subj_event != self.rc_sess_event:
            ...
            rc_subj_list = rc.export_records(
                fields=rc_subj_fields,
                raw_or_label='raw',
    ...
    def check_projects(self):
        ...
        for p in self.pa_list:
            ...
            # Look for a matching session in RedCap
            rc_scan = None
            for r in self.rc_list:
                ...
                if vuiis_label != pre_num:
                    ...
                    msg = 'date mismatch:{0}, REDCap={1}, XNAT={2}'.format(
                        pre_num,
    ...
    def do_archiving(self):
        ...
        LOGGER.info('checking Prearchive Sessions:n={0}'.format(len(self.pa_list)))
        for p in self.pa_list:
            pre_proj = p['project']
            pre_date = p['scan_date'][:10]  # get only the date without time
            pre_num = p['name']
            ...
            LOGGER.debug(
                'checking Prearchive Session:project={0},date={1},id={2}'.format(
                    pre_proj,
                    pre_date,
                    pre_num
                )
            ...
            for r in self.rc_list:
                ...
                if vuiis_label != pre_num:
                    ...
                    msg = 'project mismatch:{0}, REDCap={1}, XNAT={2}'.format(
                        pre_num, rc_proj,
                        pre_proj
                    )
            ...
            # Archive it
            LOGGER.info('archiving Session:{0} to {1}/{2}/{3}...'.format(
                pre_uri,
                proj_label,
                subj_label,
                exp_label)
    ...
    def crosscheck_redcap(self):
        ...
        # Check sessions in REDCap against XNAT Archive
        ...
        for r in self.rc_list:
            ...
            # TODO: get a whole list with single XNAT call then compare

            # Check for project
            xnat_proj = self.xnat.select.project(proj_label)
            if not xnat_proj.exists():
                msg = 'project specified in REDCap does not exist in XNAT:'
                msg += 'vuiis#={0}, sess#={1}, subj#={2}, project={3}'.format(
                    vuiis_label,
                    exp_label,
                    subj_label,
                    proj_label
            ...
            if not xnat_exp.exists():
                ...
                msg += 'vuiis#={0}, sess#={1}, project={2}'.format(
                    vuiis_label,
                    exp_label,
                    proj_label
                )
